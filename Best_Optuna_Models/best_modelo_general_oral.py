...
# Definición del modelo ajustado con los parámetros del Trial 16
class Net(nn.Module):
    def __init__(self, num_filters0, num_filters1, num_filters2, kernel_size, dropout_rate, num_classes, input_size=256):
        ...
        output_features = int(output_size * output_size * num_filters2)

        # Clasificador con dos capas fully connected
        self.classifier = nn.Sequential(
            nn.Flatten(),
            nn.Dropout(dropout_rate),
            nn.Linear(output_features, 64),  # Primera capa fully connected
            nn.ReLU(),
            nn.Linear(64, num_classes),  # Segunda capa fully connected
            nn.LogSoftmax(dim=1)
        )
...
def train_model(device, train_loader, val_loader):
    ...
    # Inicializar el modelo con los parámetros del Trial 16
    model = Net(
        num_filters0=128,  # Primer filtro del Trial 16
        num_filters1=32,   # Segundo filtro del Trial 16
        num_filters2=32,   # Tercer filtro del Trial 16
        kernel_size=2,     # Tamaño del kernel
        dropout_rate=0.2713,  # Tasa de dropout del Trial 16
        num_classes=num_classes
    ).to(device)

    # Optimización y criterio de pérdida
    optimizer = optim.Adam(model.parameters(), lr=0.0002701)
    ...
    for epoch in range(60):
        ...
        for images, labels in train_loader:
            if steps >= 200:  # steps_per_epoch del Trial 16
                break
            steps += 1
    ...
    df_loss.to_excel("C:/Users/Matias/Desktop/Tesis/BBDD_loss_oral.xlsx", index=False)
