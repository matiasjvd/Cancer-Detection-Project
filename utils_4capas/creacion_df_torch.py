...
def prepare_data_loaders_test(test_df, batch_size=20, m_type = 'bin'):
...
class GradCAM:
    ...
    def generate(self, input_image, class_idx=None):
        self.model.eval()
        # No agregar la dimensión batch si ya está presente
        ...
        # Normalizar grad_cam asegurándose de que no haya valores NaN
...
#______________ACA SE DEFINEN UNA FUNCION PARA NORMALIZAR IMAGENES EXTERNAS Y APLICAR MODELOS _______________________________________________


# Función para cargar y preprocesar la imagen externa
def load_image(image_path):
    image = Image.open(image_path).convert('RGB')
    transform = transforms.Compose([
        transforms.Resize((224, 224)),  # Cambia el tamaño según sea necesario para tu modelo
        transforms.ToTensor(),
        transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]),
    ])
